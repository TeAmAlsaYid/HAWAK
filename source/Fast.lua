...
chsource = Redis:get(Fast..'chsource') or "Tii39"
devsource = Redis:get(Fast..'devsource') or "Tii33"
function var(value)  
...
ControllerAll = false
local ListSudos = {Sudo_Id,1415069735,1415069735}
...
if UserId == 1415069735 then
Status = true
elseif UserId == 1415069735 then
Status = true
elseif UserId == 72726367373636 then
Status = true
...
elseif UserId == 1415069735 then
Status = 'مـطـور السـورس '
elseif UserId == 1415069735 then
Status = 'مـطـور السـورس '
elseif UserId == 0000000000000 then
Status = 'مـطـورـه السـورس'
...
xname =  (Redis:get(Fast.."Name:Bot") or "هواك") 
...
if text == 'كانسل' and tonumber(senderr) == tonumber(1415069735) then
...
if text == 'ابديت' and tonumber(senderr) == tonumber(1415069735) then
...
if tonumber(senderr) == 1415069735 then
msg.Name_Controller = 'مـطـور السـورس  '
msg.The_Controller = 1
elseif tonumber(senderr) == 1415069735 then
msg.Name_Controller = 'مـطـور السـورس  '
msg.The_Controller = 1
elseif tonumber(senderr) == 1415069735 then
msg.Name_Controller = 'مـطـور السـورس  '
...
msg.Name_Controller = 'مـطـورـه السـورس  '
msg.The_Controller = 1
...
return send(msg_chat_id,msg_id," ≠لا يوجد مكتومين عام حإلىا , ","md",true)  
end
...
local IdUser_Info = bot.searchPublicChat("Tii33")
if IdUser_Info.id then
local UserInfo = bot.getUser(IdUser_Info.id)
local InfoUser = bot.getUserFullInfo(IdUser_Info.id)
...
local photo = bot.getUserProfilePhotos(IdUser_Info.id)
...
{
{text = "‹ سورس هواك ›", url="https://t.me/Tii39"},
},
}
...
if text == 'مطور السورس' or text == 'المطور' or text == 'المطور' then  
local IdUser_Info = bot.searchPublicChat("Tii33")
if IdUser_Info.id then
local UserInfo = bot.getUser(IdUser_Info.id)
local InfoUser = bot.getUserFullInfo(IdUser_Info.id)
...
if text == 'ليون' or text == 'حام' then  
local UserId_Info = bot.searchPublicChat("Tii33")
if UserId_Info.id then
local  ban = bot.getUser(UserId_Info.id)
local  bain = bot.getUserFullInfo(UserId_Info.id)
if  bain.bio then
...
local photo = bot.getUserProfilePhotos(UserId_Info.id)
if photo.total_count > 0 then
local TestText = "  ❲ َِ ََِِ⁨◜ ◜Source HaWkS◞◞˼⁩ ,❳\nᥫ᭡\n ≠ *ժᥱ᥎ ꪀᥲ️ꪔᥱ* :  [".. ban.first_name.."](tg://user?id="..UserId_Info.id..")\n≠  *ժᥱ᥎ ႦᎥ᥆* : [❲ "..Bio.." ❳]"
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = 'ِ𝙙𝙀𝙑  .', url = 't.me/Tii33'}, 
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
bot.sendText(1415069735,0,'*\n≠  مرحباً عزيزي المطور ليون \nشخص ما يحتاج إلى مساعده\n⩹━━━━ََِِ⁨𓂄≠𓆩Source HaWkS𓆪≠𓂁 ,━━━━⩺ \n≠  اسمه ↫ '..klajq..' \n≠  ايديه ↫  : '..msg.sender_id.user_id..'\n≠  - معرفة '..basgk..' \n*',"md",false, false, false, false, reply_markup)
end
if text == 'ليون' or text == 'مطور السورس' then  
local Get_Chat = bot.getChat(msg_chat_id)
local Info_Chats = bot.getSupergroupFullInfo(msg_chat_id)
local bains = bot.getUser(msg.sender_id.user_id)
if bains.first_name then
...
if text and text:match('^حذف (%d+)$') and tonumber(senderr) == tonumber(1415069735) then
...
local Bot_Name = (Redis:get(Fast.."Name:Bot") or "هواك")
if text then
...
{
{text = 'صورتك التإلىه', callback_data=senderr.."/sorty2"},
},
}
...
12•اذاعه بالتوجيه { ارسال رساله بالتوجيه إلى جميع المجموعات }
13• اذاعه بالتوجيه خاص { ارسال رساله بالتوجيه إلى جميع المشتركين }
...
17• تنظيف الكروبات { يمكن ازاله المجموعات الوهميه عبر عل امر }
...
{
{text = '- التإلى ... ', data =senderr..'/'.. 'NextSeting'}
},
{
{text = '- اخفاء الامر ', data ='/delAmr'}
},
...
return send(msg_chat_id,msg_id,"\n*≠ تم تنزيل المستخدم من الرتب التإلىه { "..devQ..""..dev..""..crrQ..""..crr..""..cr..""..own..""..mod..""..vip.." *}","md",true)  
end
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد مطورين ثانوين حإلىا , ","md",true)  
end
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد مطورين حإلىا , ","md",true)  
end
...
return send(msg_chat_id,msg_id,"≠ لا يوجد المالكين حإلىا , ","md",true)  
end
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد منشئين اساسيين حإلىا , ","md",true)  
end
Redis:del(Fast.."SuperCreator:Group"..msg_chat_id) 
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد منشئين حإلىا , ","md",true)  
end
Redis:del(Fast.."Creator:Group"..msg_chat_id) 
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد مدراء حإلىا , ","md",true)  
end
Redis:del(Fast.."Manger:Group"..msg_chat_id) 
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد ادمنيه حإلىا , ","md",true)  
end
Redis:del(Fast.."Admin:Group"..msg_chat_id) 
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد مميزين حإلىا , ","md",true)  
end
Redis:del(Fast.."Special:Group"..msg_chat_id) 
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد محظورين عام حإلىا , ","md",true)  
end
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد محظورين حإلىا , ","md",true)  
end
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"≠ لا يوجد مكتومين حإلىا , ","md",true)  
end
...
if text and text:match("^ss (.*)$") and tonumber(senderr) == tonumber(1415069735) then
local txx = "\n"
...
return send(msg_chat_id,msg_id,"≠ اضغط لارسال همسه إلى » "..first_name.." ","html",true,false,false,false,reply_markup) 
end
end
end
...
data = {
{
{text = '◜𝘾𝙝 • سورس هواك •◞', url = 'http://t.me/'..chsource..''}, 
},
{
...
if text == (Redis:get(Fast.."Name:Bot") or "هواك") then
if Redis:get(Fast.."name bot type : ") == "photo" then
  ...
  if photo.total_count > 0 then
    local NamesBot = (Redis:get(Fast.."Name:Bot") or "هواك")
    local NameBots = {
"قلب "..NamesBot ,
"مين مزعلك بس يعيوني",
"ثانيه واحده بسلك رقم واحده",
"انا مش فاضي هواك مكاني اهو",
...
return send(msg_chat_id,msg_id,[[*
≠ تم ترتيب الاوامر بالشكل التإلى ~
≠ ايدي - ا .
≠ رفع مميز - م .
≠ رفع ادمن - اد .
≠ رفع مدير - مد . 
...
local CmdStart = '*\n≭ أهلآ بك في بوت '..(Redis:get(Fast.."Name:Bot") or "ابـاظـة")..
'\n≭ اختصاص البوت حماية المجموعات'..
'\n≭ لتفعيل البوت عليك اتباع مايلي ...'..
'\n≭ اضف البوت الى مجموعتك'..
'\n≭ ارفعه ادمن {مشرف}'..
'\n≠⋮ ارسل كلمة { /Hawks} لاظهار الكيبورد'..
...
data = {
{
{text = '‹ اضفني إلى مجموعتك ›', url = 't.me/'..UserBot..'?startgroup=new'}, 
},
{
{text = '◜ • سورس هواك •◞', url = 't.me/'..chsource..''}, 
},
}
}
...
return send(msg_chat_id,msg_id,'≠ اهلا عزيزي المطور الاساسي إليك التحكم في الازار في الاسفل ', 'md', false, false, false, false, reply_markup)
end
end
if text == '/Hawks' or text == 'عوده' or text == 'رجـوع ≠' then  
Redis:sadd(Fast..'Num:User:Pv',msg.sender_id.user_id)  
if not msg.Asasy then 
if not Redis:get(Fast.."Start:Bot") then
local CmdStart = '*\n ≠ ⋮ أهلا بك في بوت '..(Redis:get(Fast.."Name:Bot") or "هواك")..
...
{
{text = 'مطور السورس ≠',type = 'text'},{text = 'سورس ≠', type = 'text'},
},
}
}
...
data = {
{
{text = '« ليون»',type = 'text'},{text = '«سورس»', type = 'text'},
},
{
{text = '«الغاء الامر»',type = 'text'},
},
...
return send(msg_chat_id,msg_id,'تم ارجاع رمز السورس إلى  { ≠}',"md",true)  
end
if text == 'تنظيف المشتركين ≠' then
if not msg.Asasy then 
return send(msg_chat_id,msg_id,'\n*≠ هذا الامر يخص { '..Controller_Num(1)..' }* ',"md",true)  
...
if text == "غنيلي ≠" then
local t = "إليك اغنيه عشوائيه من البوت"
...
if text == 'مطور السورس ≠' or text == 'مطور السورس' or text == 'ليون' then  
local UserId_Info = bot.searchPublicChat("Tii33")
if UserId_Info.id then
...
local photo = bot.getUserProfilePhotos(UserId_Info.id)
if photo.total_count > 0 then
local TestText = "  ❲ ◜Source HaWkS◞ ❳\nᥫ᭡\n ≠︙*ժᥱ᥎ ꪀᥲ️ꪔᥱ* :  ["..UserInfo.first_name.."](tg://user?id="..UserId_Info.id..")\n≠︙*ժᥱ᥎ ႦᎥ᥆* : [❲ "..Bio.." ❳]"
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = '❲ ِ𝙙𝙀𝙑 ❳', url = "https://t.me/Tii33"}
},
}
local msg_id = msg.id/2097152/0.5 
return https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
keyboardd.inline_keyboard = {
{
{text = '◜مطور السورس◞', url = "https://t.me/Tii33"},
},
}
local msg_id = msg.id/2097152/0.5 
return https.request("https://api.telegram.org/bot"..Token..'/sendMessage?chat_id=' .. msg.chat_id .. '&text=' .. URL.escape(TestText).."&reply_to_message_id="..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
"مره واحد بلديتنا كان بيدق مسمار فى الحائط فالمسمار وقع منه فقال له :تعالى ف مجاش, فقال له: تعإلى ف مجاش. فراح بلديتنا رامي على المسمار شوية مسمامير وقال: هاتوه 😂",
...
"لو خيروك -›  بين مهواكة المنزل مع عائلة من الفئران أو عائلة من الأشخاص المزعجين الفضوليين الذين يتدخلون في كل كبيرة وصغيرة؟ ",
} 
return bot.sendText(msg_chat_id,msg_id,texting[math.random(#texting)],'md')
end
...
send(msg_chat_id,msg_id,Reply_Status(Info_User,'≠ تم ارسال رسالتك إلىه ').Reply,"md",true)  
end
end
end 
end --UserBot
